  ...
  while (1)
    {
      ...
      check ((client_socketfd
              = accept (in_socketfd, (struct sockaddr *)&client_addr,
...
in4_socket (struct in_addr *sin4_addr, in_port_t sin4_port)
{
  ...
  check ((in4_socketfd = socket (AF_INET, SOCK_STREAM, 0)) == -1,
         "ipv4: socket creation failed.\n");

  check (setsockopt (in4_socketfd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
                     &opt, sizeof (opt)),
  ...
  check (bind (in4_socketfd, (struct sockaddr *)&in4_sockaddr,
               sizeof (in4_sockaddr)),
         "ipv4: bind failed.\n");
...
in6_socket (struct in6_addr *sin6_addr, in_port_t sin6_port)
{
  ...
  check ((in6_socketfd = socket (AF_INET6, SOCK_STREAM, 0)) == -1,
         "ipv6: socket creation failed.\n");

  check (setsockopt (in6_socketfd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
                     &opt, sizeof (opt)),
         "ipv6: socket option setting failed.\n");

  check (
      setsockopt (in6_socketfd, IPPROTO_IPV6, IPV6_V6ONLY, &opt, sizeof (opt)),
  ...
  // memcpy (&in6_sockaddr.sin6_addr, sin6_addr, sizeof (sin6_addr));

  check (bind (in6_socketfd, (struct sockaddr *)&in6_sockaddr,
               sizeof (in6_sockaddr)),
         "ipv6: bind failed.\n");
